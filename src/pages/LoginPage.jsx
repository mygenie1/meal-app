...
export default function LoginPage() {
  ...
  async function handleAppleLogin() {
    ...
    // 성공하면 onAuthStateChange 가 화면을 전환하므로 여기 상태는 실패/취소 때만 의미가 있다.
    // 취소는 조용히 로그인 화면 유지(문구 없음), 실제 실패만 안내.
    if (!res?.ok && res.reason === 'supabase-error') {
      ...
      <div className="w-full pb-12">
        ...
        <section id="start" className="scroll-mt-6 pt-2 px-6">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-[13px] text-warm-light mt-1">
              {isNative() ? 'Apple · 카카오 · 이메일로 간편하게' : '카카오 또는 이메일로 간편하게'}
            </p>
          </div>

          {/* ── Apple로 로그인 (iOS 네이티브 전용) ──────────────────────────
              앱스토어 4.8: 제3자 소셜 로그인(카카오)을 제공하면 개인정보 보호 대안 로그인이 필요.
              ★ HIG 준수: 다른 소셜 로그인과 동등한 크기(py-4)로, 그보다 "위"에 노출한다.
                 탭 안에 숨기면 동등 노출 위반 소지가 있어 탭 스위처 바깥에 둔다.
                 로고/문구는 애플 규정 그대로(변형·재색상 금지). 검정 스타일 사용.
              웹/안드로이드는 isNative()=false → 렌더되지 않음(웹 Apple 로그인은 P6). */}
          {isNative() && (
            <>
              <button
                onClick={handleAppleLogin}
                disabled={appleLoading || loading}
                className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl font-semibold text-base bg-black text-white transition-transform active:scale-[0.99] disabled:opacity-60"
              >
                {appleLoading ? (
