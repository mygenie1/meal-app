...
const TYPE_INFO = {
  ...
  image_link: { label: '이미지링크', cls: 'bg-purple-50 text-purple-600' },
  ad:         { label: '광고',     cls: 'bg-amber-50 text-amber-600' },
}
...
function BannersContent({ payload }) {
  ...
  const [formSlot, setFormSlot] = useState(null)   // 폼이 열린 slot
  const [editingId, setEditingId] = useState(null) // null=신규, string=수정
  ...
  async function handleSave() {
    ...
    if ((form.type === 'image_link' || form.type === 'ad') && !form.image_url) {
      alert('이미지를 업로드해주세요')
      return
    }
    setSaving(true)
  ...
  async function handleToggle(id) {
    try {
      ...
      if (data.error) { alert(data.error); return }
      // 같은 slot의 모든 배너 is_active 갱신 (toggle_active는 slot내 단일 활성 보장)
  ...
  function FormPanel() {
    return (
      <div className="bg-cream-50 rounded-2xl border border-cream-200 p-4 mt-3">
        ...
        {/* 제목 */}
        <div className="mb-3">
          <p className="text-xs text-warm-light mb-1.5">제목{form.type === 'info' ? ' (필수)' : ' (선택)'}</p>
          <input
            type="text"
            value={form.title}
        ...
        {/* 이미지 (image_link / ad 타입) */}
        {(form.type === 'image_link' || form.type === 'ad') && (
          <div className="mb-3">
            <p className="text-xs text-warm-light mb-1.5">이미지 (필수)</p>
            {form.image_url ? (
              <div className="flex items-center gap-3">
                <img src={form.image_url} alt="" className="h-14 w-28 object-cover rounded-xl border border-cream-200 shrink-0" />
                <button
        ...
        {/* 링크 URL (image_link / ad 타입) */}
        {(form.type === 'image_link' || form.type === 'ad') && (
          <div className="mb-3">
            <p className="text-xs text-warm-light mb-1.5">링크 URL (클릭 시 이동)</p>
            <input
              type="url"
              value={form.link_url}
            ...
            <div className="border border-cream-200 rounded-xl overflow-hidden">
              {form.type === 'info' ? (
                ...
                <div className="relative bg-cream-100">
                  ...
                  {form.type === 'ad' && (
                    <span className="absolute top-1.5 right-1.5 text-[10px] bg-black/40 text-white px-1.5 py-0.5 rounded">광고</span>
                  )}
                </div>
              ) : null}
            </div>
      ...
      <div>
        <div className={`bg-white rounded-2xl shadow-sm p-4 flex items-center gap-3 ${
          ...
          <div className="flex-1 min-w-0">
            {banner.image_url ? (
              <div className="flex items-center gap-2">
                <img src={banner.image_url} alt="" className="h-8 w-14 object-cover rounded-lg border border-cream-100 shrink-0" />
          ...
          <div onClick={e => e.stopPropagation()} className="flex items-center gap-1 shrink-0">
            <button
              onClick={() => isEditing ? cancelForm() : openEdit(banner)}
    ...
    return (
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between px-1">
          ...
          <button
            onClick={() => isCreating ? cancelForm() : openCreate(slotKey)}
        ...
        {/* 배너 목록 */}
        {slotBanners.map(b => <BannerRow key={b.id} banner={b} />)}
      </div>
    )
  }
    ...
    <div className="min-h-svh bg-stone-100">
      ...
      {!isSuper ? (
        ...
        <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
          ...
          {!loading && !error && (
            <>
              <SlotSection slotKey="calendar_top" />
              <div className="border-t border-stone-200" />
              <SlotSection slotKey="ingredients_bottom" />
            </>
          )}
          ...
          <div className="bg-amber-50 rounded-2xl p-4 text-xs text-amber-800 space-y-1">
            ...
            <p>• <strong>이미지링크·광고</strong>: 이미지 클릭 시 새 탭으로 링크 이동</p>
            <p>• 같은 위치에 하나만 켤 수 있어요. 새 배너를 켜면 기존 배너가 자동으로 꺼집니다.</p>
            <p>• 이미지 없이 끄면 앱에서 기존 통계 카드(달력) 또는 빈 공간(재료)으로 복귀합니다.</p>
