...
ingredients    id, space_id(FK), type(toBuy|remaining), text, qty(integer default 1),
               done, created_at
...
- **남은재료**: 체크박스 없음 — QtyStepper(수량) + 삭제만
  ...
  - ingredients.qty 컬럼(integer default 1)으로 저장
- **집밥 연동 (재료 차감)**:
...
| 재료 수량 기능 | ingredients.qty 컬럼, QtyStepper 컴포넌트, 남은재료 체크 제거(수량+삭제만), 살것 체크 유지 |
