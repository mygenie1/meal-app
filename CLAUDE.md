...
│   │                                    loadAllSpaceData() → 각 space meals/ingredients 백그라운드 로드
...
│   │   │   ├── PhotoGallery.jsx         터치 스와이프 사진 갤러리 (도트 인디케이터, n/total 뱃지)
...
│   │   │   ├── MealCard.jsx             식사 기록 카드 (사진 썸네일 + 끼니/태그 뱃지)
...
│   │                                    Canvas로 최대 1200px 압축 (JPEG 0.82)
│   │                                    날짜별 자동 묶음 → 태그/끼니 선택 후 일괄 저장
│   │
│   └── pages/
│       ├── HomePage.jsx                 홈 피드 (통계 카드 + 최근 기록 피드)
│       │                                FeedCard: PhotoGallery + 끼니/태그 뱃지
...
meals         id, space_id(FK), date, title, restaurant_name, location, lat, lng,
              rating, review, memo, tag, photo(base64 단일), photos(base64[] 배열),
...
- `row.photos` 배열 있으면 그대로 사용
- 없으면 `row.photo` 단일값을 `[row.photo]`로 래핑 (하위 호환)
- `meal_time` → `mealTime`
...
### 사진 여러 장
- 최대 5장, `photos: string[]` 배열로 DB 저장
- `PhotoGallery` 컴포넌트: 터치 스와이프 + 도트 인디케이터 + n/total 뱃지
- 홈 피드 `FeedCard`, `MealDetailModal` (full-bleed), `MealCard` 썸네일 모두 반영
- 하위 호환: 구형 레코드(`photo` 단일 문자열)도 자동으로 배열로 변환

### 사진 일괄 등록 (BulkPhotoUpload)
- 스페이스 화면 "사진 일괄 등록" 버튼 → Modal
- 사진 다중 선택 → EXIF 바이너리 스캔(ArrayBuffer 첫 64KB, 외부 라이브러리 없음)으로 날짜 추출
- Canvas로 최대 1200px / JPEG 0.82 품질로 압축
...
- **사진 Supabase Storage로 이전** — 현재 base64 문자열로 DB 저장 중 (용량 큼). `meal_photos` 테이블 이미 설계됨. Storage 업로드 후 URL만 저장하는 방식으로 전환 필요
