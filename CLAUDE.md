...
### RLS 정책
- **space_members 기반 권한 관리** (`supabase-rls-migration.sql` 실행 필요)
...
- 기존 NULL-owner 스페이스 복구: boot() 시 localStorage space_id로 space_members에 직접 INSERT 후 재로드

### 환경변수 (.env)
```
VITE_SUPABASE_URL=https://jsesigubkqnddcjqusjv.supabase.co
VITE_SUPABASE_ANON_KEY=...
VITE_KAKAO_MAP_KEY=...   ← 카카오 식당 자동완성 (없으면 일반 텍스트 입력으로 폴백)
```
...
- **식당 자동완성 (RestaurantSearchField)**
  - `VITE_KAKAO_MAP_KEY` 있으면: 카카오 로컬 API, 350ms 디바운스
...
| 스페이스 권한 강화 | space_members 테이블 + RLS 정책 + join_space_by_code RPC (supabase-rls-migration.sql) |
...
### 우선순위 높음
- **카카오 API 키 발급 및 연동** — `VITE_KAKAO_MAP_KEY` 미설정 시 식당 자동완성 비활성. Kakao Developers에서 REST API 키 발급 후 `.env`와 Vercel 환경변수에 추가 필요
- **RLS SQL 실행** — `supabase-rls-migration.sql`을 Supabase SQL Editor에서 실행해야 RLS가 실제로 적용됨
